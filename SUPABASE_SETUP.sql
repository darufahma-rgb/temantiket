...
--
-- URUTAN SETUP DEPLOYMENT:
--   1) Paste SQL ini di Supabase Dashboard → SQL Editor → Run.
--   2) Deploy 3 Edge Functions (lihat supabase/functions/README.md):
--        supabase functions deploy bootstrap     --no-verify-jwt
--        supabase functions deploy invite-member
--        supabase functions deploy remove-member
--   3) Buka https://<your-vercel-domain>/bootstrap
--      → bikin owner pertama + agency.
--   4) Login normal di /login.
--   5) (Opsional) Owner bisa invite staff via Settings → Manajemen Tim.
-- ============================================================================
...
begin
  for t in select unnest(array[
    ...
    'package_calculations','notes','pdf_templates','payments','audit_logs'
  ]) loop
...
-- ── DOMAIN TABLE POLICIES (template loop) ───────────────────────────────────
do $$
declare t text;
begin
  for t in select unnest(array[
    'trips','jamaah','jamaah_docs','packages',
    'package_calculations','notes','pdf_templates','payments'
  ]) loop
    execute format($f$
...
-- Audit logs read-only buat member
