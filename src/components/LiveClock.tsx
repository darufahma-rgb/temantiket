...
export function LiveClock({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl border border-sky-100/80 bg-white/80 backdrop-blur-sm shadow-sm"
      style={{
        background:
          "linear-gradient(135deg, #f0f9ff 0%, #f0f9ff 60%, #e0f2fe 100%)",
      }}
    >
      ...
      <div className="relative px-3 py-2.5 md:px-4 md:py-3">
        <div className="flex items-center gap-1.5 mb-1.5">
          <Clock strokeWidth={2} className="h-3.5 w-3.5 text-sky-500" />
          <span className="text-[10.5px] md:text-[11px] font-bold uppercase tracking-wider text-sky-700/80">
            Waktu Dunia
          </span>
        ...
        >
          {ZONES.map((z) => (
            <div
              key={z.timeZone}
              className={
                "rounded-xl bg-white/90 border border-sky-100 px-2 py-1.5 md:px-2.5 md:py-2 min-w-[96px] flex-shrink-0 shadow-[0_1px_2px_rgba(14,165,233,0.06)]"
              }
            >
              <div className="flex items-center gap-1">
                <span className="text-[11px] leading-none">{z.flag}</span>
                <span className="text-[9.5px] md:text-[10px] font-bold uppercase tracking-wider text-sky-600">
                  {z.short}
                </span>
              </div>
              <div className="font-mono font-extrabold text-[15px] md:text-[16px] text-sky-950 leading-tight tabular-nums mt-0.5">
                {formatTime(now, z.timeZone)}
              </div>
              <div className="text-[9.5px] md:text-[10px] text-sky-700/70 leading-tight truncate">
