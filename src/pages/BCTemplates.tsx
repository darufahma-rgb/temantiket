  ...
  return (
    ...
    <div className="hidden md:flex gap-5 p-5 xl:p-6 max-w-[1440px] mx-auto w-full">
      ...
      <Dialog open={!!viewTarget} onOpenChange={(v) => !v && setViewTarget(null)}>
        <DialogContent className="max-w-[880px] w-[95vw] max-h-[92vh] rounded-2xl mx-auto p-0 gap-0 overflow-hidden flex flex-col">
          {viewTarget && (() => {
            ...
            const catShortLabel: Record<string, string> = {
              visa_on_arrival: "Visa", visa_pelajar: "Visa", umrah: "Umrah",
              haji: "Haji", tiket_pesawat: "Tiket", general: "Umum",
            };
            return (
              <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-hidden">
                ...
                {/* ── LEFT PANEL ── */}
                <div
                  className="md:w-[46%] flex flex-col overflow-y-auto"
                  style={{ background: gradient }}
                >
                  {/* Header */}
                  <div className="p-6 pb-3 flex-shrink-0">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/70 mb-1">Broadcast Template</p>
                    <h2 className="text-[22px] font-extrabold text-white leading-snug">{viewTarget.title}</h2>
                    <div className="mt-2">
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-400/30 text-white border border-emerald-300/40">
                        ● Aktif
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mx-5 mb-3 flex-shrink-0">
                    <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                      <div className="px-4 pt-4 pb-3">
                        <WAMarkdown text={viewTarget.body} className="text-[12px]" />
                        <div className="flex items-center justify-end gap-0.5 mt-2 pt-2 border-t border-slate-100">
                          ...
                          <Check className="h-3.5 w-3.5 text-blue-500 -ml-2" />
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mx-5 mb-5 flex-shrink-0">
                    <div className="bg-black/20 backdrop-blur-sm rounded-xl px-4 py-3 flex items-center justify-between gap-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-white/80 mb-2 flex items-center gap-1.5">
                          <Tag className="h-3 w-3" /> Variabel yang tersedia
                        </p>
                        {vars.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {vars.map((v) => (
                              <span key={v} className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/20 text-white border border-white/30">
                                {"{{"}{v}{"}}"}
                              </span>
                            ))}
                          </div>
                        ) : (
                          <p className="text-[11px] text-white/60 italic">Tidak ada variabel</p>
                        )}
                      </div>
                ...
                <div className="flex-1 flex flex-col bg-white min-h-0 relative">
                  ...
                  <button
                    ...
                    className="absolute top-3 right-3 z-10 h-8 w-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors"
                  >
                    <X className="h-4 w-4" />
                  </button>
                  ...
                  <div className="px-6 pt-6 pb-4 border-b border-slate-100 flex-shrink-0">
                    <h3 className="text-[17px] font-extrabold text-[#0f1c3f]">Detail Template</h3>
                    <p className="text-[12px] text-muted-foreground mt-0.5">Informasi dan performa template broadcast ini</p>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-2">
                    {/* Info rows */}
                    {[
                      { Icon: LayoutGrid, label: "Kategori",         value: catShortLabel[viewTarget.category] ?? cat.label },
                      { Icon: Send,       label: "Tipe",             value: "Broadcast" },
                      { Icon: null,       label: "Status",           value: null,             isStatus: true },
                      { Icon: User,       label: "Dibuat oleh",      value: viewTarget.createdBy ? "Admin" : "Admin" },
                      { Icon: Calendar,   label: "Dibuat pada",      value: fmtDateTime(viewTarget.createdAt) },
                      { Icon: Pencil,     label: "Terakhir diupdate",value: fmtDateTime(viewTarget.updatedAt) },
                      { Icon: Clock,      label: "Total digunakan",  value: "—",              suffix: "kali" },
                      { Icon: Send,       label: "Total terkirim",   value: "—",              suffix: "kali" },
                    ].map(({ Icon, label, value, isStatus, suffix }) => (
                      <div key={label} className="flex items-center justify-between py-2.5 border-b border-slate-50">
                        <div className="flex items-center gap-2.5 text-[12px] text-muted-foreground">
                          {Icon
                            ? <Icon className="h-4 w-4 text-slate-400 shrink-0" />
                            : <span className="h-4 w-4 flex items-center justify-center shrink-0"><span className="h-2 w-2 rounded-full bg-emerald-400" /></span>
                          }
                          {label}
                        </div>
                        {isStatus ? (
                          <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Aktif</span>
                        ) : (
                          <span className="text-[12px] font-semibold text-[#0f1c3f]">
                    ...
                    {/* Tips Penggunaan */}
                    <div className="mt-4 mb-2 rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-3 flex items-start gap-3">
                      <Lightbulb className="h-4 w-4 text-indigo-500 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-[12px] font-bold text-indigo-800 mb-0.5">Tips Penggunaan</p>
                        <p className="text-[11.5px] text-indigo-600 leading-relaxed">
                          Gunakan variabel yang tersedia untuk membuat pesan lebih personal dan relevan dengan klien Anda.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-6 py-4 border-t border-slate-100 flex gap-3 flex-shrink-0">
                    {canEdit && (
                      <Button
                        variant="outline"
                        className="flex-1 rounded-xl h-11 gap-2 border-slate-300 text-[13px] font-semibold"
                    ...
                    <Button
                      className="flex-1 rounded-xl h-11 gap-2 bg-indigo-600 hover:bg-indigo-700 text-[13px] font-semibold"
