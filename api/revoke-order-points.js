...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!['owner', 'staff'].includes(callerMember.role)) {
      return res.status(403).json({ error: 'Hanya owner atau staff yang bisa revoke poin' });
    }

    const { orderId } = req.body || {};
    if (!orderId) return res.status(400).json({ error: 'orderId diperlukan' });

    const { error: deleteErr } = await admin.from('agent_points')
      .delete()
      .eq('order_id', orderId)
    ...
    console.error('[revoke-order-points]', e);
