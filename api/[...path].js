...
async function handleFlightSearch(req, res, _caller) {
  if (!SERPAPI_KEY) return res.status(503).json({ error: 'SERPAPI_KEY tidak dikonfigurasi di Vercel Environment Variables.' });

  const { departure_id, arrival_id, outbound_date, return_date, currency = 'IDR', adults = 1, travel_class = 1 } = req.query;
  if (!departure_id || !arrival_id || !outbound_date) {
    return res.status(400).json({ error: 'Parameter departure_id, arrival_id, dan outbound_date wajib diisi.' });
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return res.status(resp.status).json({ error: `SerpAPI error: ${resp.statusText}` });
    }
    ...
    return res.status(200).json({ results, searchedAt: new Date().toISOString() });
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  if (resource === 'flight-search'           && req.method === 'GET')  return handleFlightSearch(req, res, caller);
